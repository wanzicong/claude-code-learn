...
description: 用于创建针对 Slack 优化的动画 GIF 的知识和实用程序。提供约束、验证工具和动画概念。当用户请求为 Slack 制作动画 GIF 时使用，例如"为 Slack 制作一个 X 做 Y 的 GIF"。
...
图形应该看起来精美和有创意，而不是基本的。以下是方法：

**使用更粗的线条** - 始终为轮廓和线条设置 `width=2` 或更高。细线（width=1）看起来参差不齐且业余。

**添加视觉深度**：
- 对背景使用渐变（`create_gradient_background`）
- 分层多个形状以增加复杂性（例如，一个星形内有一个较小的星形）
...
要有创意和详细！一个好的 Slack GIF 应该看起来精美，而不是像占位符图形。
...
1. **更少的帧** - 较低的 FPS（10 而不是 20）或更短的持续时间
...
**关于用户上传的注意事项**：此技能不包括预构建的图形，但如果用户上传图像，请使用 PIL 加载和处理它 - 根据他们的请求解释他们是想直接使用它还是仅作为灵感。
