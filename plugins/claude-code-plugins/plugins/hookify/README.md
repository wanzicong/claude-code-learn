...
通过分析对话模式或根据明确指令轻松创建自定义钩子以防止不需要的行为。

## 概述

hookify 插件使创建钩子变得简单，无需编辑复杂的 `hooks.json` 文件。相反，您创建定义要监视的模式和匹配这些模式时显示的消息的轻量级 markdown 配置文件。

**主要功能：**
- 🎯 自动分析对话以查找不需要的行为
- 📝 带有 YAML 前置内容的简单 markdown 配置文件
- 🔍 用于强大规则的正则表达式模式匹配
- 🚀 无需编码 - 只需描述行为
- 🔄 无需重启即可轻松启用/禁用
...
```bash
/hookify 在我使用 rm -rf 命令时警告我
```

这会分析您的请求并创建 `.claude/hookify.warn-rm.local.md`。

### 2. 立即测试

**无需重启！** 规则在下一次工具使用时立即生效。

让 Claude 运行应该触发规则的命令：
```
运行 rm -rf /tmp/test
```
...
**如果会话记录中没有出现测试命令，这会阻止 Claude 停止**。仅在需要严格执行时启用。
...
此插件是 Claude Code Marketplace 的一部分。安装 marketplace 时应该会自动发现。
...
**规则未触发：**
1. 检查规则文件是否存在于 `.claude/` 目录中（在项目根目录，而不是插件目录）
2. 验证前置内容中的 `enabled: true`
3. 单独测试正则表达式模式
4. 规则应该立即生效 - 无需重启
